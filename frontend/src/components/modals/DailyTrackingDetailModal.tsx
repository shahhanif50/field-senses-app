...
import { SiteVisitReportViewModal } from "@/components/modals/SiteVisitReportViewModal";
...
export function DailyTrackingDetailModal({
  ...
  const [selectedReportTask, setSelectedReportTask] = useState<TaskEntry | null>(null);
  ...
  */
  ...
  return (
    ...
    <AnimatePresence>
                                ...
                                >
                                  ...
                                  <TableCell>
                                    <p className="text-sm text-muted-foreground max-w-[200px] truncate">
                                      {task.notes?.includes('siteOperationsChecklist') ? 'Report Submitted' : task.notes}
                                    </p>
                                  </TableCell>

                                  <TableCell className="text-right">
                                    <div className="flex flex-col items-end gap-2">
                                      {task.notes?.includes('siteOperationsChecklist') && (
                                        <Button
                                          variant="outline"
                                          size="sm"
                                          className="h-8 text-xs bg-blue-50 text-blue-600 border-blue-200 hover:bg-blue-100 hover:text-blue-700"
                                          onClick={(e) => {
                                            e.stopPropagation();
                                      ...
                                      {(task.notes?.includes('siteOperationsChecklist') || task.fuelExpense > 0 || task.foodExpense > 0) && (
      ...
      <SiteVisitReportViewModal
        isOpen={!!selectedReportTask}
        onClose={() => setSelectedReportTask(null)}
        reportJson={selectedReportTask?.notes || null}
        siteName={selectedReportTask?.taskName || "Site"}
        proofUrl={selectedReportTask?.proofUrl || null}
        onApprove={selectedReportTask ? () => { handleApproveProof(selectedReportTask.id); setSelectedReportTask(null); } : undefined}
        onDeny={selectedReportTask ? () => { handleRejectProof(selectedReportTask.id); setSelectedReportTask(null); } : undefined}
      />
    </AnimatePresence>
  );
}
